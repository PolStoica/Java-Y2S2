...
public class Ex2PalindromeFinder {
    ...
    public boolean isPalindromic(int number) {
        ...
        stringBuilderVariable.append(string);
        String inverse = String.valueOf(stringBuilderVariable.reverse());
    ...
    public int[] findPalindromes(int a, int b) {
        ...
        int checking=a; int palindromes=0;

        while (checking<= b){
            if (isPalindromic(checking)){
                palindromes++;
            }
            checking++;
        }
        int[] array =new int[palindromes];

        int currentNumber=a; int k=0;

        while (currentNumber<= b){
            if (isPalindromic(currentNumber)){
                array[k++] = currentNumber;
            }
            currentNumber++;
        }
        return array;
    ...
    public static void main(String[] args) {
        ...
        int f=1;
        int l=100;
        int[] array = finder.findPalindromes(f,l);
        System.out.printf("The palindrome numbers in the range %d - %d are: ",f,l);
